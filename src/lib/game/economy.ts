...
function baseTierUnits(group: CreatureGroupKey): SevenTierUnits {
  const units = CREATURES
    .filter((creature) => creature.group === group && creature.upgradeLevel === 0 && creature.tier >= 1 && creature.tier <= 7)
    .map((creature) => creature.type);

  if (units.length !== 7) {
    throw new Error(`Expected 7 base units for faction group ${group}, got ${units.length}`);
  }
...
export const FACTION_UNITS = Object.fromEntries(
  Object.entries(FACTION_GROUP_KEYS).map(([faction, group]) => [faction, baseTierUnits(group)]),
