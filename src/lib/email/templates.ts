...
export function bugReportEmail(input: BugReportInput): EmailContent {
  const { title, description, reporterName, reporterEmail, context = {} } = input;
  ...
  return {
    subject: `[My-Heroes][BUG-REPORT] ${title}`,
    html: shell(
      "Signalement de bug",
