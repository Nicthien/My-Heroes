...
export default function CombatResultModal() {
  ...
  return (
    <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/65 pointer-events-auto">
      <div className="w-[min(92vw,38rem)] rounded-xl border border-yellow-600 bg-stone-950 p-6 text-white shadow-2xl">
        <div className="text-xs uppercase tracking-[0.3em] text-yellow-500">Résultat</div>
        <h2 className="mt-2 text-2xl font-bold text-yellow-100">Combat terminé</h2>
        <div className="mt-4 rounded bg-black/40 p-3 text-sm text-stone-300">
          ...
          {result.experienceGained > 0 && <span> | XP +{result.experienceGained}</span>}
        </div>
        ...
        <button className="mt-6 rounded bg-yellow-700 px-5 py-2 font-bold text-white hover:bg-yellow-600" onClick={() => setCombatResult(null)}>
          Retour à la carte
        </button>
      </div>
    </div>
