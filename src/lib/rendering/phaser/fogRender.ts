...
export function generateFogStampTextures(scene: Phaser.Scene) {
  ...
  for (const [stampKey, textureKeys] of Object.entries(FOG_STAMP_TEXTURE_KEYS) as [FogStampKey, string[]][]) {
    textureKeys.forEach((textureKey, variant) => {
      ...
      if (stampKey === "fog-unexplored") {
        generateUnexploredCloudTexture(scene, textureKey, variant);
        return;
      }
      ...
      switch (stampKey) {
        case "fog-near":
          drawFogNearTileVisual(graphics, centerX, centerY, variant);
          break;
        case "fog-explored":
...
function generateUnexploredCloudTexture(scene: Phaser.Scene, textureKey: string, variant: number) {
  ...
  // Cumulonimbus palette: medium-dark blue-grey base (deep cloud belly, not
  // pitch-black), pale cloud body, near-white highlight on the densest peaks.
  const baseR = 0x32, baseG = 0x3e, baseB = 0x52;
  const cloudR = 0xd0, cloudG = 0xd8, cloudB = 0xe2;
  const highlightR = 0xee, highlightG = 0xf1, highlightB = 0xf7;
  ...
  for (let py = 0; py < h; py++) {
    for (let px = 0; px < w; px++) {
      ...
      data[idx + 1] = gOut;
      data[idx + 2] = bOut;
      data[idx + 3] = Math.round(envelope * 255);
    }
  }
...
// terrain readable.
function drawFogTileVisual(graphics: Phaser.GameObjects.Graphics, x: number, y: number, _explored: boolean, variant: number) {
  const variantSeedShift = (variant + 1) * 0x27d4eb2d;

  drawSoftCircularFill(graphics, x, y, 0x000000, 0.04, TILE_WIDTH / 2);
...
function drawFogNearTileVisual(graphics: Phaser.GameObjects.Graphics, x: number, y: number, variant: number) {
  const variantSeedShift = (variant + 1) * 0x27d4eb2d;
  ...
  drawSoftCircularFill(graphics, x, y, 0x000000, 0.025, TILE_WIDTH / 2);
...
function drawFogFrontierEdgeVisual(graphics: Phaser.GameObjects.Graphics, x: number, y: number, side: FogEdgeSide) {
  ...
  const featherSteps: { amount: number; color: number; alpha: number }[] = [
